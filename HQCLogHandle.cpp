...
LogHandlerPrivate::LogHandlerPrivate() {
    logDir.setPath("file/log/" + QDate::currentDate().toString("yyyy-MM") + "/" + QDate::currentDate().toString("dd")); // TODO: 日志文件夹的路径，为 exe 所在目录下的 log 文件夹，可从配置文件读取
    QString logPath = logDir.absoluteFilePath(QTime::currentTime().toString("hh-mm-ss.log")); // 获取日志的路径
    ...
    // 十分钟检查一次日志文件创建时间
    renameLogFileTimer.setInterval(1000 *  60); // TODO: 可从配置文件读取
...
void LogHandlerPrivate::openAndBackupLogFile() {
    ...
    QString logPath = logDir.absoluteFilePath(QTime::currentTime().toString("hh-mm-ss.log")); // log.txt的路径
    ...
    if (logFileCreatedDate != QDate::currentDate()) {
        ...
        QString newLogPath = logDir.absoluteFilePath(QTime::currentTime().toString("hh-mm-ss.log"));;
...
void LogHandlerPrivate::checkLogFiles() {
    ...
    if (logFile->size() > 1024*1024*g_logLimitSize){
        ...
        QString logPath = logDir.absoluteFilePath(QTime::currentTime().toString("hh-mm-ss.log")); // 日志的路径
...
void LogHandlerPrivate::messageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
    ...
    QStringList strList;
    //默认
    param1 = QString::number(context.line);
    param2 = context.function;
    log = msg;
    switch (type) {
    case QtDebugMsg:
        level = "DEBUG";
    ...
    case QtInfoMsg:
        ...
        if(msg.front() == '@'){
            strList = msg.split(" ");
            log = msg.mid(msg.indexOf(" ",msg.indexOf(" ")+1));
            param1 = strList.value(0).remove('@');
            param2 = strList.at(1);
        }
        break;
    case QtWarningMsg:
    ...
    /*(*LogHandlerPrivate::logOut) << QString("%1-[%2]-(%3-%4):%5\n")
                                    ...
                                    //.arg(fileName)
                                    .arg(context.line) //行数
                                    .arg(context.function) //函数名
                                    .arg(msg); //日志内容*/
    (*LogHandlerPrivate::logOut) << QString("%1 $ [%2] $ (%3 $ %4) $ %5\n").arg(time).arg(level).arg(param1).arg(param2).arg(log);
}
 ...
 *                                                                                                          *
 ***********************************************************************************************************/
HQCCLogHandle::HQCCLogHandle() : d(nullptr) {
}

// 给Qt安装消息处理函数
void HQCCLogHandle::installMessageHandler() {
...
// 取消安装消息处理函数并释放资源
void HQCCLogHandle::uninstallMessageHandler() {
    QMutexLocker locker(&LogHandlerPrivate::logMutex);
